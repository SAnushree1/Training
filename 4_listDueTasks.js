...
function listDueTasks(todos,people){
    const personNames = {};
    people.map(person => {
    ...
    for (const todo of todos){
        if (!isNaN(todo.due)) continue;

        if ( todo.status.toLowerCase() !== "done"){
            const dueDate = new Date(todo.due);
            if (dueDate<=todayDate){
                pendingTask.push({"id": todo.id, "title": todo.title,"assigneeName" : personNames[todo.assigneeId],"due": todo.due})
            }
        }
    }
    return pendingTask;
...
console.log(listDueTasks(todos,people));
