...
function findWorkLoad(todos,people){
    const currentWorkingHours = estimateWorkHours(people,todos);

    const personCapacitySummary = people.reduce((summary,person) => {
        const personCapacityPerSprint = person.capacityHrsPerDay * 5; 
        const currentHours = currentWorkingHours[person.name];
        if (currentHours < personCapacityPerSprint){
            summary[person.name] = "OK";
        }else{
            const workload = personCapacityPerSprint - currentHours;
            summary[person.name] = "OVER ALLOCATED" + workload + "hrs";
        }
        return summary;
    },{});

    return personCapacitySummary;
}

console.log(findWorkLoad(todos,people));
