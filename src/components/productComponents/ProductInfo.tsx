...
const ProductInfo: React.FC<ProductInfoProps> = ({ product }) => {
    const [selectedSize, setSize] = useState<number>(50);

    const calculatePrice = (basePrice: number | string, size: number) => {
        const price = typeof basePrice === 'string' ? parseFloat(basePrice) : basePrice;
        const baseSize = 50;
        return Math.floor((price / baseSize) * size);
    };
                    ...
                    <div className='flex gap-3 '>
                        {sizesAvailable.map((size) => (
                            <button
                                ...
                                className={` p-2 rounded-4xl ${selectedSize === size ? "bg-gray-400 text-white" : "border-2 text-gray-500 border-[#474747] cursor-pointer"}`}
                            >
                                {size} g
                            </button>
                        ))}
